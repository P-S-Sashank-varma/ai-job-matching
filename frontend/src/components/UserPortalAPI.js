...
export const checkExistingResume = async (userEmail, setHasExistingResume, setName, setPhone, activeTab, location) => {
  try {
    const response = await fetch(`https://ai-job-matching-zd8j.onrender.com/check-resume/${userEmail}`);
...
export const fetchSelectedJobs = async (userEmail, setSelectedJobs) => {
  try {
    const response = await fetch(`https://ai-job-matching-zd8j.onrender.com/matching-jobs/${userEmail}`);
    if (!response.ok) {
...
export const handleResumeUpload = async (
  ...
  try {
    setLoading(true);
    const uploadResponse = await fetch("https://ai-job-matching-zd8j.onrender.com/upload-resume", {
      method: "POST",
      body: formData,
    });
    ...
    await fetch(`https://ai-job-matching-zd8j.onrender.com/parse-resume/${email}`);
...
export const handleApply = async (jobId, email, hasExistingResume, setActiveTab, setLoading, selectedJobs, setSelectedJobs) => {
  ...
  try {
    setLoading(true);
    const response = await fetch("https://ai-job-matching-zd8j.onrender.com/apply-job", {
      method: "POST",
      headers: {
      ...
      jobStatuses.map(async (status) => {
        try {
          ...
          const jobResponse = await fetch(
            `https://ai-job-matching-zd8j.onrender.com/job-status/${encodeURIComponent(userEmail)}/${encodeURIComponent(status.recruiter_email)}`
          );
